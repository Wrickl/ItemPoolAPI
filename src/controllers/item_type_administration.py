...
def _get_configured_upload_data_types() -> list[str]:
...
def _build_item_type_assignment_read(
    assignment: ItemTypeContentPiece,
) -> ItemTypeContentPieceRead:
    if assignment.item_type_content_piece_id is None:
        raise HTTPException(
            status_code=500, detail="ItemTypeContentPiece ohne ID gefunden"
        )
    if (
        assignment.content_piece is None
        or assignment.content_piece.content_piece_id is None
    ):
        raise HTTPException(status_code=500, detail="Zugehoeriges ContentPiece fehlt")
    if (
        assignment.content_piece.data_type is None
        or assignment.content_piece.data_type.data_type_id is None
    ):
...
def _build_item_type_detail(
    item_type: ItemType, session: Session
) -> ItemTypeDetailRead:
    ...
    assignments = session.exec(
        ...
        .order_by(ItemTypeContentPiece.item_type_content_piece_id)
    ).all()
    return ItemTypeDetailRead(
        ...
        content_pieces=[
            _build_item_type_assignment_read(assignment) for assignment in assignments
        ],
    )


@router.get(
    "/getAllDataTypes", response_model=list[DataTypeRead], tags=["ContentTypes"]
)
...
@router.get(
    "/getUploadDataTypes", response_model=list[str], tags=["ContentTypes"]
)
async def get_upload_data_types() -> list[str]:
    """Gibt die Datentypen zurueck, die im Create-Form als File-Upload gerendert werden."""
    return _get_configured_upload_data_types()


@router.post(
    "/syncAvailableDataTypes", response_model=list[DataTypeRead], tags=["ContentTypes"]
)
...
async def create_item_type(
    item_type_data: ItemTypeCreate, session: Session = Depends(get_session)
):
    """Legt einen neuen ItemType an."""
    existing = session.exec(
        select(ItemType).where(ItemType.name == item_type_data.name)
    ).first()
    if existing:
        raise HTTPException(
            status_code=409,
            detail=f"ItemType '{item_type_data.name}' existiert bereits",
        )
...
@router.get(
    "/getAllItemTypes", response_model=list[ItemTypeDetailRead], tags=["ContentTypes"]
)
...
@router.get(
    "/getAllContentPieces", response_model=list[ContentPieceRead], tags=["ContentTypes"]
)
async def get_all_content_pieces(session: Session = Depends(get_session)):
    ...
    content_pieces = session.exec(
        select(ContentPiece).order_by(ContentPiece.name)
    ).all()
    return [_build_content_piece_read(cp) for cp in content_pieces]


@router.post(
    "/createContentPiece", response_model=ContentPieceRead, tags=["ContentTypes"]
)
async def create_content_piece(
    content_piece_data: ContentPieceCreate, session: Session = Depends(get_session)
):
...
async def assign_content_piece_to_item_type(
    ...
    if item_type is None:
        raise HTTPException(
            status_code=404, detail=f"ItemType {item_type_id} nicht gefunden"
        )
    ...
    if content_piece is None:
        raise HTTPException(
            status_code=404,
            detail=f"ContentPiece {assignment_data.content_piece_id} nicht gefunden",
        )
    ...
    existing_assignment = session.exec(
        select(ItemTypeContentPiece).where(
            ...
            ItemTypeContentPiece.usage_area == assignment_data.usage_area,
        )
    ).first()
    if existing_assignment is not None:
        raise HTTPException(
            ...
            detail=(
                "ContentPiece"
                f"{assignment_data.content_piece_id} ist ItemType {item_type_id} "
                f"im Bereich '{assignment_data.usage_area}' bereits zugeordnet"
            ),
        )
    ...
    assignment = ItemTypeContentPiece(
        ...
        usage_area=assignment_data.usage_area,
